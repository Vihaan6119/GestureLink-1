...
const SignLanguageDetector: React.FC<SignLanguageDetectorProps> = ({ 
  ...
  // Improved sign language phrases with higher accuracy simulation
  const signPhrases = [
    "Hello, how are you?",
    "Good morning!",
    "Thank you very much.",
  ...
  const initializeComputerVision = async () => {
    ...
    try {
      ...
      toast.error('Failed to initialize AI detection. Using simulation mode.');
    } finally {
  ...
  const startDetection = async () => {
    ...
    if (cvDetector && videoRef.current) {
      ...
      const interval = setInterval(() => {
        if (videoRef.current && cvDetector) {
          const detectedSign: DetectedSign | null = cvDetector.detectSigns(videoRef.current);
      ...
      }, 500); // Check every 500ms for real-time detection

      setDetectionInterval(interval);
      toast.success('AI sign language detection started');
    } else {
      // Fallback to simulation if CV fails
      const interval = setInterval(() => {
        const randomPhrase = signPhrases[Math.floor(Math.random() * signPhrases.length)];
        setCurrentTranslation(randomPhrase);
        onTranslation(randomPhrase, 'sign');
      ...
      setDetectionInterval(interval);
      toast.info('Using simulation mode - show hand gestures to camera');
    }
  };
  ...
  const startSpeechRecognition = () => {
    ...
    const SpeechRecognition = (window as Window & typeof globalThis & { SpeechRecognition?: typeof SpeechRecognition; webkitSpeechRecognition?: typeof SpeechRecognition }).SpeechRecognition || (window as Window & typeof globalThis & { webkitSpeechRecognition?: typeof SpeechRecognition }).webkitSpeechRecognition;
          ...
          <CardContent>
            ...
            {cvDetector && mode === 'sign-to-text' && (
              <div className="mt-3 text-sm text-gray-600">
                <p><strong>Supported Signs:</strong> Hello, Thank you, Please, Yes, No, I love you</p>
                <p className="text-xs mt-1">Move your hands in front of the camera - detection based on motion patterns</p>
              </div>
            )}
          </CardContent>
        </Card>
