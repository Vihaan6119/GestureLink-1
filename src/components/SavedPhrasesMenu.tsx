...
interface SavedPhrasesMenuProps {
  phrases: string[];
...
const SavedPhrasesMenu: React.FC<SavedPhrasesMenuProps> = ({
  phrases,
  onPhraseAdd,
  onPhraseRemove,
  onPhraseUse,
      ...
      <div className="ml-6 space-y-3">
        <div className="flex gap-2">
          <Input
            ...
            className="text-sm"
          />
          <Button onClick={handleAddPhrase} size="sm" className="px-2">
        ...
        <div className="max-h-64 overflow-y-auto space-y-2">
          {phrases.length === 0 ? (
            <p className="text-center text-gray-500 text-sm py-4">No saved phrases yet</p>
          ) : (
            phrases.map((phrase, index) => (
              <div
                key={index}
                className="group flex items-center justify-between p-2 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors"
              >
                <p 
                  className="flex-1 cursor-pointer text-sm"
                  style={{ fontSize: `${Math.max(textSize - 2, 12)}px` }}
...
export default SavedPhrasesMenu;